  ...
  const socialPlatforms = [
    {
      ...
      href: `https://twitter.com/intent/tweet?text=${encodeURIComponent(title)}&url=${encodeURIComponent(url)}${hashtags.length > 0 ? `&hashtags=${hashtags.join(',')}` : ''}`,
    ...
    <div className="flex items-center space-x-2">
      ...
      {isWebShareSupported && (
        <button
          ...
          className="p-2 text-gray-600 hover:text-blue-600 transition-colors dark:text-gray-400 dark:hover:text-blue-400"
          title="Share"
        >
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {socialPlatforms.map((platform) => (
        <button
          ...
          className={`p-2 text-gray-600 transition-colors rounded-lg ${platform.color} dark:text-gray-400`}
